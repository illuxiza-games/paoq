...
const basic = Math.min(window.innerWidth, window.innerHeight, 640) - 10;

export const boardSize = {
    board: basic,
    boardEdge: basic * 0.0,
    boardGrid: basic * 0.25,
    pieceRadius: basic * 0.1
}
...
export const getIndexByBoard = (data: number) => {
    return Math.min(3, Math.max(0, Math.round((data - boardSize.boardEdge) / boardSize.boardGrid) - 1));
}
